...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_rays', type=int, default=256, help="num rays sampled per image for each training step")
    ...
    parser.add_argument('--indicator_steps', type=int, default=16)
    parser.add_argument('--upsample_steps', type=int, default=16, help="num steps up-sampled per ray (only valid when NOT using --cuda_ray)")
    ...
    if opt.test:
        ...
        metrics = [PSNRMeter(), LPIPSMeter(device=device)]
        trainer = Trainer('ngp', opt, model, device=device, workspace=opt.workspace, optimizer=optimizer, criterion=criterion, ema_decay=0.95, fp16=opt.fp16, lr_scheduler=scheduler, scheduler_update_every_step=False, metrics=metrics, use_checkpoint=opt.ckpt, eval_interval=50)
